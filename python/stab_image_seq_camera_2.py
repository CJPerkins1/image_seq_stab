...
def open_images_and_stabilizer():
    ...
    vid_capture_string = "normalized_images/" + image_prefix + 't%03d.tif'
...
# The second camera has unpredictable brightness, so this function normalizes
# the images. Without it the stabilization has trouble tracking on a subset of 
# the images.
def normalize_brightness():
    """
    Normalize the mean brightness of an image.

    """

    brightness_level = 200

    # For some reason the stabilization only works in a small number of 
    # cases if I normalize all the images and save them, then run the 
    # stabilization on the saved images. I checked the array shapes etc. 
    # and I can't pin down the reason, so I'm doing it this way so I don't 
    # lose my mind, even if it's a bit slower.

    # Make a temp directory to store the normalized images
    new_dir = "normalized_images"
    if not os.path.exists(new_dir):
        os.mkdir(new_dir)
    else:
        raise ValueError(f"Error: {new_dir} already exists.")
    ...
    #image_prefix = sorted(glob.glob('*.tif'))[0][:-8]
    #print("input_array is: ", type(image_array))
    #print("input_array dimensions are: ", image_array.shape)
...
def stabilize_and_write_images(stabilizer, image_seq, image_names, image_num):
    ...
    while True:
        ...
        # This is a little sloppy, but if it's out of frames in the .read() 
        # then it will break here (happens at the end of the image seq).
        if frame is None:
             break

        # Normalizes the brightness of the frame, due to inconsistency in the 
        # second camera.
...
### Clean saved normalized image directory
def clean_normalized_dir():
    directory_path = os.path.join(os.getcwd(), "normalized_images")
...
def main():
    ...
    # Normalizing the image brightness
    ...
    # Clean up the saved normalized images directory
    clean_normalized_dir()
